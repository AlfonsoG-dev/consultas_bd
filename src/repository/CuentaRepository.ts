...
export class CuentaRepository implements DbQueryModel{
    ...
    get read_all(): Promise<CuentaTypes[] | undefined> {
        return new Promise((resolve, reject)=>{
            this.con.any_execute('select nombre, email, create_at, update_at from `consulta`.cuentas')
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    get_cuentas_by_user(user_id: number) : Promise<CuentaTypes[] | undefined>{
        return new Promise((resolve, reject)=>{
            this.con.any_execute('select cuenta_id, nombre, email, create_at, update_at from `consulta`.cuentas where user_id=?', user_id)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    read_by_email(email: string, id:number): Promise<CuentaTypes[] | undefined> {
        return new Promise((resolve, reject) =>{
            this.con.any_execute('select nombre, email from `consulta`.cuentas where email=? and user_id=?', email, id)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    read_by_name(nombre: string, id: number): Promise<CuentaTypes[] | undefined> {
        return new Promise((resolve, reject)=>{
            this.con.any_execute('select nombre, email from `consulta`.cuentas where nombre = ? and user_id=?', nombre, id)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    ...
    read_by_user_id(user_id: number, nombre: string, email: string): Promise<CuentaTypes[] | undefined>{
        return new Promise((resolve, reject)=>{
            this.con.any_execute('select cuenta_id, nombre, email from `consulta`.cuentas where user_id=? && nombre=? || email =?', user_id, nombre, email)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    read_by_id(id: number, user_id: number): Promise<CuentaTypes[] | undefined> {
        return new Promise((resolve, reject) =>{
            this.con.any_execute('select nombre, email, create_at, update_at from `consulta`.cuentas where cuenta_id=? and user_id=?', id, user_id)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    insert_register(nCuenta: Omit<Cuenta, "get_id"| "set_id" | "get_update_at" | "get_password">): Promise<ResultSetHeader | undefined> {
        return new Promise((resolve, reject)=>{
            this.con.any_execute('insert into `consulta`.cuentas (nombre, email, user_id, create_at) values(?, ?, ?, ?)', nCuenta.get_nombre, nCuenta.get_email, nCuenta.get_user_id, nCuenta.get_create_at)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    update_register(nCuenta: Omit<Cuenta, "get_create_at" | "get_password" | "set_id">): Promise<ResultSetHeader | undefined> {
        return new Promise((resolve, reject)=>{
            this.con.any_execute('update `consulta`.cuentas set nombre=?, email=?, update_at=? where cuenta_id=? and user_id=?', nCuenta.get_nombre, nCuenta.get_email, nCuenta.get_update_at, nCuenta.get_id, nCuenta.get_user_id)
            .then((res) => resolve(res))
            .catch((err) => reject(err))
        })
    }
    delete_register(cuenta_id: number): Promise<ResultSetHeader | undefined> {
        return new Promise((resolve, reject)=>{
            this.con.any_execute('delete from `consulta`.cuentas where cuenta_id=?', cuenta_id)
            .then((res) => resolve(res) )
            .catch((err) => reject(err))
        })
    }
